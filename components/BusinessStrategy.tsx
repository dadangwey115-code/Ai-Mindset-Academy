...
export const BusinessStrategy: React.FC<{ language: Language }> = ({ language }) => {
  ...
  return (
    <div className="py-24 bg-[#050505] border-y border-white/5 overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <div className="space-y-10">
              <div className="flex gap-6 group">
                ...
                <div>
                  <h4 className="text-xl font-bold text-white mb-2">{s.s1Title}</h4>
                  <p className="text-gray-400">{s.s1Desc}</p>
                </div>
              </div>
              <div className="flex gap-6 group">
                ...
                <div>
                  <h4 className="text-xl font-bold text-white mb-2">{s.s2Title}</h4>
                  <p className="text-gray-400">{s.s2Desc}</p>
                </div>
              </div>
              <div className="flex gap-6 group">
                ...
                <div>
                  <h4 className="text-xl font-bold text-white mb-2">{s.s3Title}</h4>
                  <p className="text-gray-400">{s.s3Desc}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="relative bg-zinc-900/50 border border-white/10 p-8 md:p-10 rounded-[40px] backdrop-blur-2xl shadow-2xl overflow-hidden ring-1 ring-white/5">
               <div className="flex justify-between items-center mb-10">
                  ...
                  <div className="flex gap-2">
                    {roadmapSteps.map((_, i) => (
                      <div key={i} className={`w-1.5 h-1.5 rounded-full transition-all duration-500 ${activeStep === i ? 'bg-blue-500 w-4' : 'bg-white/10'}`} />
                    ))}
                  </div>
               </div>
                  ...
                  {roadmapSteps.map((step, i) => (
                    <div 
                      ...
                      className={`flex items-center gap-5 p-4 rounded-2xl border transition-all duration-700 ${
                        activeStep === i 
                        ? 'bg-white/5 border-white/20 translate-x-2 scale-[1.02] shadow-lg shadow-black/20' 
                        : 'bg-transparent border-transparent opacity-30 grayscale blur-[1px]'
                      }`}
                    >
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${step.bg} ${step.color} border border-white/5 shadow-inner`}>
                        <step.icon size={22} className={activeStep === i ? 'animate-bounce-subtle' : ''} />
                      </div>
                      <div className="flex-1">
                        <div className="flex justify-between items-center mb-1">
                          <span className={`text-sm font-bold uppercase tracking-widest ${activeStep === i ? 'text-white' : 'text-gray-500'}`}>
                            {step.label}
                          </span>
               ...
               <div className="mt-12 space-y-4">
                  ...
                  <a 
                    ...
                    className="w-full py-4 bg-white/5 hover:bg-white/10 text-gray-300 border border-white/10 rounded-2xl font-bold transition-all flex items-center justify-center gap-3 group"
                  >
